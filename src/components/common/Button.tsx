...
function Button({ variant, disabled = false, onClick, children }: ButtonProps): JSX.Element {
  const variantClasses = (variant: string): string => {
    switch (variant) {
      case "login":
        if (disabled) {
      ...
      case "input":
        return "flex w-83 h-32 justify-center items-center gap-10 flex-shrink-0 rounded-4 border-2 border-gray-D9D9 text-violet text-center font-Pretendard text-lg font-bold";
      default:
        return "";
    }
  };
