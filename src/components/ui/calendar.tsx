...
const classNames = {
  ...
  nav_button: cn(buttonVariants({ variant: "outline" }), "size-20 bg-transparent p-0 opacity-50 hover:opacity-100"),
  ...
  day: cn(
    ...
    "size-20 aria-selected:opacity-100 hover:bg-violet-F1EF focus:outline-none focus:ring focus:ring-violet-300",
  ),
...
function Calendar({ className, showOutsideDays = true, ...props }: CalendarProps) {
  ...
  return (
    <DayPicker
      ...
      components={{
        IconLeft: ({ ...props }) => <ChevronLeft className="size-20" />,
        IconRight: ({ ...props }) => <ChevronRight className="size-20" />,
      }}
      {...props}
    />
  );
